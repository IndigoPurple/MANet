    ...
    try:
        ...
        for epoch in range(config.checkpoint_epoch, config.max_epoch+1):
            ...
            for batch_index, batched_sample in enumerate(dataloader_train):
                ...
                img1_pred, flow_1_1, _, _, _ = net(img0, img1, img2)
                ...
                if iter_ % 1000 ==0:
                    save_batchimages(img1_pred.data.cpu().numpy(), visualization_path+str(iter_)+'_predction_', count=iter_*config.batch_size_train)
                    save_batchimages(img1_gt.data.cpu().numpy(), visualization_path+str(iter_)+'_gt_', count=iter_*config.batch_size_train)
                    save_batchflows(flow_1_1.data.cpu().numpy(), visualization_path+str(iter_)+'_flow_1_1_', count=iter_*config.batch_size_train)
                    save_batchflows_np(flow_1_1.data.cpu().numpy(), visualization_path+str(iter_)+'_flow_1_1_', count=iter_*config.batch_size_train)
            ...
            if (epoch + 1) % config.testing_epoch == 0:
                with torch.no_grad():
                    ...
                    for batch_index, batched_sample in enumerate(dataloader_test):
                        ...
                        img1_pred, flow_10_1, _, _, _ = net(img0, img1, img2)
                        loss = criterion(img1_pred, img1_gt)
                        psnr = psnr_fun(img1_pred, img1_gt)
                        ...
                        if batch_index*config.batch_size_test<100:
                            ...
                            save_batchflows(flow_10_1.data.cpu().numpy(), visualization_path+'flow_10_1_', count=batch_index*config.batch_size_test)
                            save_batchflows_np(flow_10_1.data.cpu().numpy(), visualization_path+'flow_10_1_', count=batch_index*config.batch_size_test)
                        else:
                            break
...
def my_test_net(net, dataloader_test, config):
    model_name = get_name2(config.model_name, config)
    snapshot_path = config.snapshot_root + model_name + '/'
    snapshot_file = snapshot_path + config.snapshot_file
...
def get_model_flags_v1():
    ...
    ''' config '''
    parser.add_argument('--model_name', type = str, default='single_flow', help='model_name')
    ...
    parser.add_argument('--snapshot_file', type = str, default='INTERRUPTED.pth', help= 'snapshot filename')
...
if __name__ == '__main__':
    ...
    net = Models.FlowNet_MultiscaleWarp_2Frame()
    if config.use_gpu:
