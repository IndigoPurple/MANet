    ...
    try:
        ...
        for epoch in range(config.checkpoint_epoch, config.max_epoch+1):
            ...
            for batch_index, batched_sample in enumerate(dataloader_train):
                ...
                img1_pred, flow_1, attention1 = net(img0, img1, img2)
                ...
                if iter_ % 1000 ==0:
                    flow_1_np = flow_1.data.cpu().numpy()
                    attention1_np = attention1.data.cpu().numpy()
                    ...
                    save_batchflows(flow_1_np[:,0:2,:,:], visualization_path+'flow_1_1_', count=iter_*config.batch_size_train)
                    save_batchflows(flow_1_np[:,2:4,:,:], visualization_path+'flow_1_2_', count=iter_*config.batch_size_train)
                    save_batchflows(flow_1_np[:,4:6,:,:], visualization_path+'flow_1_3_', count=iter_*config.batch_size_train)
                    save_batchflows(flow_1_np[:,6:8,:,:], visualization_path+'flow_1_4_', count=iter_*config.batch_size_train)
                    save_batchflows_np(flow_1_np, visualization_path+'flow_1_1_', count=iter_*config.batch_size_train)
                    save_attention_np(attention1_np[:,0:2,:,:], visualization_path+'attention_1_1_', count=batch_index*config.batch_size_test)
                    save_attention_np(attention1_np[:,2:4,:,:], visualization_path+'attention_1_2_', count=batch_index*config.batch_size_test)
                    save_attention_np(attention1_np[:,4:6,:,:], visualization_path+'attention_1_3_', count=batch_index*config.batch_size_test)
                    save_attention_np(attention1_np[:,6:8,:,:], visualization_path+'attention_1_4_', count=batch_index*config.batch_size_test)
                ...
                with torch.no_grad():
                    ...
                    for batch_index, batched_sample in enumerate(dataloader_test):
                        ...
                        if batch_index*config.batch_size_test<20:
                            ...
                            save_attention_np(attention1.data.cpu().numpy(), visualization_path+'attention_1_', count=batch_index*config.batch_size_test)
...
def my_test_net(net, dataloader_test, config):
    ...
    visualization_path = snapshot_path + config.visualization_folder + '/'
    ...
    with torch.no_grad():
        ...
        for batch_index, batched_sample in enumerate(dataloader_test):
            ...
            if batch_index*config.batch_size_test<100:
                ...
                save_batchflows(flow_1_np[:,0:2,:,:], visualization_path+'flow_1_1_', count=batch_index*config.batch_size_test)
                save_batchflows(flow_1_np[:,2:4,:,:], visualization_path+'flow_1_2_', count=batch_index*config.batch_size_test)
                save_batchflows(flow_1_np[:,4:6,:,:], visualization_path+'flow_1_3_', count=batch_index*config.batch_size_test)
                save_batchflows(flow_1_np[:,6:8,:,:], visualization_path+'flow_1_4_', count=batch_index*config.batch_size_test)
                save_attention_np(attention1_np[:,0:2,:,:], visualization_path+'attention_1_1_', count=batch_index*config.batch_size_test)
                save_attention_np(attention1_np[:,2:4,:,:], visualization_path+'attention_1_2_', count=batch_index*config.batch_size_test)
                save_attention_np(attention1_np[:,4:6,:,:], visualization_path+'attention_1_3_', count=batch_index*config.batch_size_test)
                save_attention_np(attention1_np[:,6:8,:,:], visualization_path+'attention_1_4_', count=batch_index*config.batch_size_test)
...
def get_model_flags_v1():
    ...
    ''' config '''
    parser.add_argument('--model_name', type = str, default='multi_flow_simple_efficient', help='model_name')
    ...
    parser.add_argument('--visualization_folder', type = str, default='visualization', help= 'visualization_folder folder')
...
if __name__ == '__main__':
    ...
    net = Models.FlowNet_MultiscaleWarp_2Frame_multi_Simple_efficient(4)
    ...
    if config.test_or_train=='test':
        my_test_net(net, dataloader_train, config)
